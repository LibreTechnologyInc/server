...
describe('ClientManager', () => {
    ...
    it('should create a new client overriding existing if force set', async () => {
        const manager = new ClientManager();
        const clientA = await manager.newClient('foobar');
        assert(manager.hasClient(clientA.id));
        const closePromise = new Promise(resolve => manager.getClient(clientA.id).once('close', resolve));
        const clientB = await manager.newClient('foobar', { force: true });
        await closePromise;
        assert(manager.hasClient(clientB.id));
        assert(clientB.id === clientA.id);
        manager.removeClient(clientB.id);
